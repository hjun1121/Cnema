	...
	<div style = "width: 90%; margin: 25px auto;">
		...
		<div style="clear: both; border: 1px solid #f0f0f0; height: 300px;">
			...
			<c:forEach items="${pageList}" var="page">
				<a href="pageMain?page_num=${page.page_num}"><img style="width: 30%; margin-right: 30px;" alt="${page.page_name}_logo" src="../resources/community_logo/${page.fileName}"> ${page.page_name}</a>
			</c:forEach>
		</div>
		</c:if>
		<!-- 추천 페이지 LIST -->
		<div style="clear: both; border: 1px solid #f0f0f0; height: 300px; margin-top: 10px;">
			...
			<c:forEach items="${recommendPage}" var="recommend" varStatus="i">
				<c:if test="${i.index < 3}">
					<a href="pageMain?page_num=${recommend.page_num}"><p><img style="width: 30%; margin-right: 30px;" alt="${recommend.page_name}_logo" src="../resources/community_logo/${recommend.fileName}"> ${recommend.page_name}</p></a>
				</c:if>
			</c:forEach>
		</div>
	</div>
